...
P_id=1
file_contour=dir+'Contour_Piece_'+str(P_id)+'.txt'

file_In_point=dir+'In_point_Piece_'+str(P_id)+'.txt'

file=open(file_contour)
X=[]
Y=[]
rl=file.readline()
nbp=0
...
file.close()
file=open(file_In_point)
for i in range(nbp_in):
    rl=file.readline().split('\t')
    X_in_pts.append(int(rl[0]))
    Y_in_pts.append(int(rl[1]))

plt.scatter(X_in_pts,Y_in_pts)
...
def function_Pts_In_Env(num):
    ...
    k=0
    pts_in_env=0
    vect_inter=np.array([[np.sqrt(6)/3],[np.sqrt(3)/3]])
    for i in range(nbp_in):
        P=point(X_in_pts[i],Y_in_pts[i])
        plt.scatter(P[0],P[1],c='b')
        for j in range(len(seg_ind)):
            A = seg_seq[seg_ind[j]][0]
            B = seg_seq[seg_ind[j]][1]
            D = P + vect_inter
            I=Intersect(A,B,P,D)[0]
            t=Intersect(A,B,P,D)[1]
            print(I,t)
    ...
    if pts_in_env>0 and pts_in_env<0.8*nbp_in:
        plt.savefig(dir+'RD_frame_pts_count//'+str(pts_in_env)+'_'+str(num)+'.png')
        print(pts_in_env)
    plt.close()
